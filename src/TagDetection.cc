...
void TagDetection::draw_with_id(cv::Mat& image) const {
  // use corner points detected by line intersection
  std::pair<float, float> p1 = p[0];
  std::pair<float, float> p2 = p[1];
  std::pair<float, float> p3 = p[2];
  ...
  if(id % 2 == 0){
    ...
    cv::circle(image, cv::Point2f(cxy.first, cxy.second), 8, cv::Scalar(0,255,0,0), 2);


    cv::Point vertices[4];
    vertices[0] = cv::Point2f(p1.first, p1.second);
    vertices[1] = cv::Point2f(p2.first, p2.second);
    vertices[2] = cv::Point2f(p3.first, p3.second);
    vertices[3] = cv::Point2f(p4.first, p4.second);

    cv::fillConvexPoly(image, vertices, 4, cv::Scalar(0, 255, 0, 0));
  }
  ...
  cv::putText(image, strSt.str(),
              cv::Point2f(cxy.first + 10, cxy.second + 10),
              cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0,128,255),2);
}
